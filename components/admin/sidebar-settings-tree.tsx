...
import { Settings, ChevronDown, ChevronRight, Mail, KeyRound, Database, Server, Bot } from "lucide-react"
    ...
    <div className="flex flex-col">
      ...
      {isOpen && (
        <div className="flex flex-col mt-1 ml-6 pl-3 border-l-2 border-slate-100 space-y-1 animate-in slide-in-from-top-1 fade-in duration-200">
          ...
          <Link 
            href="/admin/settings/ai-gateway" 
            className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${pathname === '/admin/settings/ai-gateway' ? 'text-brand-blue-deep bg-white shadow-sm border border-slate-100' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'}`}
          >
