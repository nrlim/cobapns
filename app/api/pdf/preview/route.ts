 ...
 * GET /api/pdf/preview?type=TRYOUT|IQ|PSYCHOLOGY&id=<examResultId>
 *
 * Identical to /api/pdf/report but uses Content-Disposition: inline
 * so the browser renders the PDF inside an <iframe> for preview.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // KEY DIFFERENCE: inline instead of attachment → browser renders it
    ...
    return NextResponse.json(
      { error: "Gagal membuat pratinjau laporan. Silakan coba lagi." },
      { status: 500 }
    )
  }
}
